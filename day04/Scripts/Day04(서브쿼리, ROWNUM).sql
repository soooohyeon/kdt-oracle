...
-- EMPLOYEES 테이블에서 SALARY를 내림차순으로 정렬한 후 ROWNUM을 붙여 조회
SELECT ROWNUM, E.* FROM EMPLOYEES E
...
-- 1. 내부쿼리1(가장 안쪽) : EMPLOYEES 테이블의 SALARY 기준 내림차순 정렬
SELECT SALARY
FROM EMPLOYEES
ORDER BY SALARY DESC;

-- 2. 내부쿼리2 : ROWNUM 정렬된 급여(내부쿼리1) 데이터에 순번(ROWNUM) 부여
...
-- 3. 메인쿼리 : 내부쿼리2에 부여된 순번(RN)을 6 ~ 10까지로 조건 검사 후 새로운 순번 부여
SELECT ROWNUM, e2.*
